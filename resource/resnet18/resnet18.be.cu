...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_2_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_2_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_1_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_relu, float* __restrict__ placeholder, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_1_kernel2_device(bgemm, T_relu, placeholder, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_1_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_1_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_2_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_2_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_3_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_add, float* __restrict__ placeholder) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_3_kernel2_device(bgemm, T_add, placeholder);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_multiply_add_nn_re_11882905421691233276__kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_multiply_add_nn_re_11882905421691233276__kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_2_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_add, float* __restrict__ placeholder) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_2_kernel2_device(bgemm, T_add, placeholder);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_3_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_3_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_1_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_1_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_2_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_relu, float* __restrict__ placeholder, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_2_kernel2_device(bgemm, T_relu, placeholder, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_1_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_1_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_relu, float* __restrict__ placeholder, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_kernel2_device(bgemm, T_relu, placeholder, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_multiply_add_nn_re_11882905421691233276__kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_relu, float* __restrict__ placeholder, float* __restrict__ placeholder1, float* __restrict__ placeholder2) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_multiply_add_nn_re_11882905421691233276__kernel2_device(bgemm, T_relu, placeholder, placeholder1, placeholder2);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_multiply_add_nn_re_11882905421691233276__kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_multiply_add_nn_re_11882905421691233276__kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_1_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_add, float* __restrict__ placeholder) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_1_kernel2_device(bgemm, T_add, placeholder);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_2_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_add_nn_relu_2_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_kernel2(int* preempted, int* task_slot, float* __restrict__ bgemm, float* __restrict__ T_add, float* __restrict__ placeholder) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_kernel2_device(bgemm, T_add, placeholder);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_3_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_3_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_1_kernel1(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack, float* __restrict__ bgemm) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_1_kernel1_device(placeholder, data_pack, bgemm);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_contrib_conv2d_winograd_without_weight_transform_add_2_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ data_pack) {
    if (*preempted) return;
    fused_nn_contrib_conv2d_winograd_without_weight_transform_add_2_kernel0_device(placeholder, data_pack);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_add_10_kernel0(int* preempted, int* task_slot, float* __restrict__ T_add, float* __restrict__ placeholder, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_add_10_kernel0_device(T_add, placeholder, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
