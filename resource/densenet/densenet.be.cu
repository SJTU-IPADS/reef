...
extern "C" __global__ void fused_nn_avg_pool2d_add_nn_relu_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ T_relu, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_nn_avg_pool2d_add_nn_relu_kernel0_device(placeholder, T_relu, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_avg_pool2d_add_nn_relu_1_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ T_relu, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_nn_avg_pool2d_add_nn_relu_1_kernel0_device(placeholder, T_relu, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
...
extern "C" __global__ void fused_nn_avg_pool2d_add_nn_relu_2_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ T_relu, float* __restrict__ placeholder1) {
    if (*preempted) return;
    fused_nn_avg_pool2d_add_nn_relu_2_kernel0_device(placeholder, T_relu, placeholder1);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        

extern "C" __global__ void fused_nn_avg_pool2d_kernel0(int* preempted, int* task_slot, float* __restrict__ placeholder, float* __restrict__ tensor) {
    if (*preempted) return;
    fused_nn_avg_pool2d_kernel0_device(placeholder, tensor);
    if (threadIdx.x + threadIdx.y + threadIdx.z == 0)
        atomicAdd(task_slot, 1);
}        
